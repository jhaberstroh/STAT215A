...
<<temp_humid_slicer, cache=FALSE, echo=FALSE,dpi=100, fig.cap='Analysis of humidity/temperature correlation across all days, 1:20pm-2:00pm', fig.height=3, fig.width=6 >>=
same_time <- filter(log.edge, time.dmin >=800 & time.dmin <= 840)
p1 <- ggplot(same_time, aes(humid_temp, humidity)) +
  stat_smooth(method='loess', span=.3, formula = y ~ poly(x,1)) +
  geom_point(alpha=.01) +
  ggtitle("Linear fit with bandwidth .2")
p2 <- ggplot(same_time, aes(humid_temp, humidity)) +
